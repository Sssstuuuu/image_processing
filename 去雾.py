...
def getAtomsphericLight(darkChannel,img,meanMode = False, percent = 0.001):
	...
	for i in range(0,height):
		for j in range(0,width):
			oneNode = (i,j,darkChannel[i,j])
			nodes.append(oneNode)	
	# 排序
	nodes = sorted(nodes, key = lambda node: node[2],reverse = True)
	...
	if int(percent*size) == 0:
		for i in range(0,3):
			if img[nodes[0].x,nodes[0].y,i] > atomsphericLight:
				atomsphericLight = img[nodes[0][0],nodes[0][1],i]
	...
	if meanMode:
		...
		for i in range(0,int(percent*size)):
			for j in range(0,3):
				sum = sum + img[nodes[i][0],nodes[i][1],j]
	...
	for i in range(0,int(percent*size)):
		for j in range(0,3):
			if img[nodes[i][0],nodes[i][1],j] > atomsphericLight:
				atomsphericLight = img[nodes[i][0],nodes[i][1],j]
	return atomsphericLight
...
img = imread('1.jpg')
img_res = getRecoverScene(img, omega = 0.95,t0 = 0.1 , blockSize = 15 , meanMode = False,percent = 0.001)
plt.imshow(img)
plt.show()
plt.imshow(img_res)
plt.show()
